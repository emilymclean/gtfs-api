...
class ByteNetworkGraphGeneratorV2(Writer):
    ...
    def __init__(
        ...
        self.trip_id_to_trip_id_index = {t.id: i for i, t in enumerate(trips)}
        ...
        self.trip_route_to_stop_edges: Dict[int, Dict[Tuple[int, int], RouteStopEdge]] = {}
    ...
    def _connect_graph(self, reverse=False) -> bytes:
        ...
        for trip_id in self.trip_ids:
            mapping_bytes.extend(str_to_bytes(trip_id))
        for service_id in self.service_ids:
            mapping_bytes.extend(str_to_bytes(service_id))

        for node_index, node in enumerate(self.nodes):
            associated_edges = edges[node_index] if node_index in edges else []
            connective_edges = ((self.trip_stop_to_route_edges[node_index]
                                if node_index in self.trip_stop_to_route_edges else
                                {}) |
                                (self.trip_route_to_stop_edges[node_index]
                                if node_index in self.trip_route_to_stop_edges else
                                {}))
    ...
    def generate(self, output_folder: Path):
        ...
        self._write(reverse_graph, output_folder.joinpath("v2/network-graph-reverse.eng"))
    ...
    def _create_stop_to_route_edge(
        ...
        if route_index in self.trip_route_to_stop_edges:
            route_edges = self.trip_route_to_stop_edges[route_index]
        else:
            route_edges = {}
        ...
        if k not in route_edges:
            route_edge = RouteStopEdge(stop_index)
            route_edges[k] = route_edge
            self.trip_route_to_stop_edges[route_index] = route_edges
